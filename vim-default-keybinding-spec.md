# Stock Vim Default Keybinding Behavior

## Source Baseline
- Vim baseline: local `vim --version` reports Vim 9.1, patches 1-1752, compiled January 16, 2026.
- Help baseline: local `index.txt` reports last change August 6, 2025; local `defaults.vim` reports last change July 13, 2025.
- Intent: this is a "planning" document for reproducing stock Vim behavior before subsetting it for the application.

## Scope
- Baseline: stock Vim behavior anchored to the local Vim 9.1 help index (`:help index`) and the shipped `defaults.vim` used by `vim --clean`.
- This is exhaustive for Vim's built-in key-driven behavior cataloged in the help index: Insert, Normal, text objects, window commands, bracket commands, `g`/`z` prefixes, operator-pending, Visual, command-line editing, Terminal-Job mode, and an appendix of `:` commands.
- This is not exhaustive for buffer-local mappings introduced by filetype plugins, GUI menus, or user config. `vim --clean` enables filetype detection/plugins/indent, so filetype-specific mappings can still exist; they are intentionally excluded here.
- Treat the top sections as the implementation-oriented interpretation layer and the appendix sections as the authoritative raw command inventory.

## Core Command Grammar
- Modes: Normal is the command hub; Insert/Replace/Virtual Replace input text; Visual/Select operate on a live selection; Operator-pending waits for a motion or text object; command-line mode edits `:`, `/`, `?`, and `!` prompts; Terminal-Job mode forwards most keys into an embedded terminal job.
- Counts: a numeric prefix usually repeats or scales a command. Counts commonly multiply across operators and motions, e.g. `2d3w` means delete six words.
- Registers: `"{register}` targets the next delete, yank, change, or put. Macros use `q{register}` to record and `@{register}` to replay.
- Operators: verbs like `d`, `c`, `y`, `g~`, `gu`, `gU`, `!`, `=`, `<`, `>`, and `zf` combine with motions or text objects.
- Motions: cursor movement commands are reusable as operator targets. Many are linewise, characterwise, screen-line-aware, search-based, or structure-based.
- Text objects: commands like `iw`, `aw`, `ip`, `a"`, `i(`, `at`, etc. are only meaningful after an operator or in Visual mode.
- Stateful navigation: search history, marks, jumplist, changelist, tag stack, alternate file, folds, and registers all affect later key behavior.
- Option-sensitive behavior: keys such as `j`/`k` vs `gj`/`gk`, `~`, `%`, `*`, `#`, `K`, `=` and search keys depend on options like `wrap`, `tildeop`, `matchpairs`, `iskeyword`, `keywordprg`, `equalprg`, `incsearch`, `nrformats`, and filetype settings.

## `vim --clean` Deltas And Behavioral Modifiers
| Item | Effect |
| --- | --- |
| `set nocompatible` | Uses Vim behavior rather than legacy Vi compatibility. |
| `map Q gq` and `sunmap Q` | In Normal mode, `Q` formats text like `gq` instead of entering Ex mode. In Select mode, `Q` is explicitly unmapped. The raw help index still documents the built-in `Q -> Ex` behavior, so this delta should override that entry for a `vim --clean` baseline. |
| `inoremap <C-U> <C-G>u<C-U>` | Insert-mode `Ctrl-U` starts a fresh undo block before deleting backward within the current inserted line. |
| `set mouse=...` when mouse support exists | Mouse click, selection, and scroll behaviors become active in supported terminals. The exact setting depends on terminal type. |
| `set ttimeout` and `ttimeoutlen=100` | `Esc`-prefixed keycode parsing is time-sensitive, which matters for distinguishing bare `Esc` from special keys. |
| `set incsearch` | `/` and `?` update incrementally while the pattern is being typed when timing support is available. |
| `set nrformats-=octal` | `Ctrl-A` and `Ctrl-X` stop treating leading-zero numbers as octal by default. |
| `filetype plugin indent on` | Core keybindings stay the same, but indentation, formatting, completion, and some buffer-local mappings may vary by filetype. Those filetype-local mappings are out of scope for this document. |
| `syntax on` | Not a keymap change, but it affects the visible/editorial context in which many motions and text objects are used. |

## Implementation Buckets
- Tier 1 core: Normal motions, operators, text objects, counts, registers, undo/redo, insert/replace, search, marks, macros, and Visual modes.
- Tier 2 editor surface: window commands, tab commands, command-line editing, scrolling, folding, and bracket/`g`/`z` command families.
- Tier 3 edge features: spell actions, tag stack commands, Terminal-Job mode, command-line window, Select mode, and the full `:` command surface.

## Exhaustive Reference Appendix
The sections below are mechanically normalized from Vim's local help index so the inventory stays complete. When a top-level delta conflicts with a raw entry, prefer the top-level delta. The Ex appendix includes feature-gated commands that may not exist in every compiled Vim build.
